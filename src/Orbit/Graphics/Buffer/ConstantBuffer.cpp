...
ConstantBuffer::ConstantBuffer( size_t size )
{
	...
	switch( context_impl_ptr->index() )
	{
		...
		case( render_context_impl_index_v< _RenderContextImplOpenGL > ):
		{
			...
			if( ( context_impl->embedded && context_impl->opengl_version < Version( 3 ) ) || ( !context_impl->embedded && context_impl->opengl_version < Version( 3, 1 ) ) )
			{
...
void ConstantBuffer::Update( void* dst, size_t location, const void* data, size_t size )
{
	switch( m_impl.index() )
	{
		...
		{
			auto& functions = std::get_if< _RenderContextImplOpenGL >( RenderContext::GetCurrent()->GetImplPtr() )->functions.value();
