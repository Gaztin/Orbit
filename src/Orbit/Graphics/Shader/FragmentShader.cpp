...
FragmentShader::FragmentShader( const Asset& asset )
{
	...
	switch( context_impl_var.index() )
	{
		...
		{
			auto& impl         = m_impl.emplace< Private::_FragmentShaderImplOpenGL >();
		...
		{
			auto&                  impl         = m_impl.emplace< Private::_FragmentShaderImplD3D11 >();
			...
			if( D3DCompile( asset.GetData(), asset.GetSize(), NULL, macros, nullptr, "main", "ps_5_0", flags, 0, &pixel_data, &pixel_errors ) != S_OK )
			{
...
FragmentShader::~FragmentShader( void )
{
	switch( m_impl.index() )
	{
		...
		case( unique_index_v< Private::_FragmentShaderImplOpenGL, Private::FragmentShaderImpl > ):
		{
			auto& impl         = std::get< Private::_FragmentShaderImplOpenGL >( m_impl );
